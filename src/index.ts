...
if (command === "dump-categories") {
  ...
  if (paths.length < 2) {
    throw new Error(
      `Two positional args are expected: parse-excel <input-path> <output-path>`,
    );
  }
  ...
  const parsingConfiguration =
    parser === "wise" ? WiseParsingConfiguration : BankParsingConfiguration;
  ...
  const rollupTransactionsOptions = cli(rollupTransactionsDefinitions, {
    argv,
  });
